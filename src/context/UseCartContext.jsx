...
const UseCartContext = ({ children }) => {
  ...
  const addToCart = (dataProduct, contador) => {
    ...
    if (item) {
      // existe item?
      if (contador > 1) {
        // ademas, me esta llegando una cantidad seleccionada
        item.quantity += contador; // entonces a la cantidad de mi producto(que ya esta en el carrito) le sumo la cantidad seleccionada
      } else {
        // si no me estaa llegando una cantidad seleccionada => aumento en uno la cantidad de mi producto
        item.quantity++;
        setCart([...cart]); // para que se actualice la cantidad
      }
    } else {
      // NO existe item(no esta en el carrito)
      if (!contador) {
        // ademas, no existe contador(no hay una cantidad seleccionada)
        setCart([...cart, dataProduct]); // entonces se agrega el producto con cantidad 1 y lo que ya esta en el carro
      } else {
        // si hay contador y ademas el producto no esta en carrito
